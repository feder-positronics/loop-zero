...
from loopzero.review import risk as module

root = Path("/home/marcin/dev/intelflo")


@pytest.fixture(autouse=True)
def _isolated_authority_state(tmp_path: Path):
    from loopzero.kernel import authority as kernel_authority
    from loopzero.kernel import authority_store

    previous = kernel_settings.settings
    configured = kernel_settings.KernelSettings(
        state_root=tmp_path / "coordinator-state",
        audit_root=Path(".audit"),
        env_prefix="INTELFLO",
    )
    kernel_settings.configure(configured)
    kernel_authority.settings = configured
    authority_store.settings = configured
    try:
        yield
    finally:
        kernel_settings.configure(previous)
        kernel_authority.settings = previous
        authority_store.settings = previous


def load_module(name: str) -> ModuleType:
    if name == "pr_publish_risk":
        from loopzero.delivery import _publish_risk

        return _publish_risk
...
def test_historical_changed_path_replay_corpus_has_no_t2_escape() -> None:
    ...
    corpus_shas = sorted(
        {case[field] for case in corpus["cases"] for field in ("base_sha", "head_sha")}
    )
    require_historical_commits(root, corpus_shas)

    for case in corpus["cases"]:
        envelope = module.compute_review_risk(root, case["base_sha"], case["head_sha"])
        artifact = module.parse_review_risk(envelope["review_risk_json"])
        assert artifact["tier"] == case["expected_tier"], case["name"]
        assert artifact["security_trigger_paths"] == case["security_trigger_paths"]
        assert not (case["expected_tier"] == "T2" and artifact["tier"] == "T0")
