...
When a required GitHub Actions check has failed or been cancelled, readiness may
read workflow runs to see whether a newer run on the same head is still active.
Fine-grained GitHub tokens need Actions read permission for that lookup.
