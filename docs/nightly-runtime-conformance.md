...
Operator reference, extracted from the project README. Workflow configuration is
owned by [nightly-conformance.yml](../.github/workflows/nightly-conformance.yml);
credential renewal procedures are in [nightly credentials](nightly-conformance.md).
The release-bootstrap note below is historical context, not a current setup step.
...
[nightly conformance credentials](nightly-conformance.md) for the exact
